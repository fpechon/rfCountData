...
#' @param ..., two or more objects of class \code{rfCountData}, to be combined into one.
#' @return An object of class \code{rfCountData}.
#' @note  The confusion, \code{err.rate}, \code{mse} and \code{rsq} components
#'  (as well as the corresponding components in the test compnent, if exist) of the combined object will be \code{NULL}.
...
combine <- function(...) {
   ...
   padm0 <- function(x, len) rbind(x, matrix(0, nrow=len-nrow(x),
                                             ncol=ncol(x)))
   rflist <- list(...)
   areForest <- sapply(rflist, function(x) inherits(x, "rfCountData")) 
   if (any(!areForest)) stop("Argument must be a list of rfCountData objects")
   ...
   if (! all(numvars[1] == numvars[-1]))
       stop("Unequal number of predictor variables in the rfCountData objects.")
   for (i in seq_along(vlist)) {
       if (! all(vlist[[i]] == vlist[[1]]))
           stop("Predictor variables are different in the rfCountData objects.")
   }
